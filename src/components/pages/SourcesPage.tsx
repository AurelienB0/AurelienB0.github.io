  ...
  return (
    <div className="space-y-20">
      <section id="biblio" className="scroll-mt-48">
        ...
        <div className="space-y-4 bg-white rounded-lg p-8 border border-gray-200 shadow-sm">
          {bibliography.map((ref, index) => (
            <p key={index} className="text-foreground/80 text-sm leading-relaxed pl-6 -indent-6">
              {ref}
            </p>
          ))}
        </div>
      ...
      <section id="ia" className="scroll-mt-48">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-1.5 h-10 bg-indigo-600 rounded-full" />
          <h2 className="pb-0 text-indigo-700">Utilisation d'intelligence artificielle</h2>
        </div>
        <div className="bg-white rounded-lg p-8 border-l-4 border-indigo-600 shadow-sm">
          <p className="text-foreground/80 leading-relaxed">
            Notre groupe a eu recours à certains outils d'IA générative pour superviser la création du site internet. N'ayant jamais fait de web, nous avons fait l'architecture du site à l'aide de LLM puis avons mis en page tout notre travail et nos recherches après avoir appris les bases du framework React. Nos recherches ont toutes été réalisées grâce à Google Scholar, le journal en ligne The Conversation, les bases de données universitaires HAL UM et BIU, la base de données du CAIRN ainsi que l'outil NotebookLM pour suggérer des ressources.
          </p>
        </div>
      </section>
      ...
      <section id="mentions" className="scroll-mt-48">
        ...
        <div className="bg-white rounded-lg p-8 space-y-4 text-foreground/80 leading-relaxed border border-gray-200 shadow-sm">
          <p>
            Le site web suivant est un exercice réalisé par des élèves de 3ème année de l'école Polytech Montpellier dans le cadre du cours d'introduction au système d'information du département IG. Ces sites sont le résultat de travail d'étudiants et sont mis en ligne pour des raisons pédagogiques et didactiques; une fois en ligne, ils ne sont plus mis à jour.
          </p>
          <p>
            Vos réactions et commentaires sont très bienvenus. Pour signaler une erreur, réagir à un contenu ou demander une modification, merci d'écrire aux élèves et à la responsable de l'enseignement (isabelle.bourdon@umontpellier.fr).
          </p>
          <p>
            Polytech Montpellier décline toute responsabilité pour les erreurs et les imprécisions que peuvent contenir ces sites.
          </p>
        </div>
      </section>
      ...
      <section id="equipe" className="scroll-mt-48">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {teamMembers.map((member, index) => (
            <div key={index} className="bg-white hover:bg-gray-50 transition-all duration-300 rounded-lg p-8 text-center border-2 border-gray-200 hover:border-indigo-400 shadow-sm hover:shadow-md">
